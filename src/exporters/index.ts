...
export type ExportFormat = 'postman' | 'insomnia' | 'all';
...
export async function writeExports(scan: ScanResult, options: WriteExportsOptions): Promise<string[]> {
  ...
  const formats = options.format === 'all' ? ['postman', 'insomnia'] as const : [options.format];

  if (formats.includes('postman')) {
    const file = path.join(outputDir, 'routier-postman.json');
    await writeFile(file, `${JSON.stringify(generatePostmanCollection(scan, {
  ...
  if (formats.includes('insomnia')) {
    ...
    await writeFile(file, `${JSON.stringify(generateInsomniaExport(scan, {
      baseUrl: options.baseUrl,
      groupBy: options.groupBy,
      sort: options.sort,
    }), null, 2)}\n`);
